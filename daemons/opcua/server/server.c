...
static void printIntegerName(UA_QualifiedName *myIntegerName)
{
  printf("myIntegerName: nsIndex=%d length=%d data=%8s\n", myIntegerName->namespaceIndex, myIntegerName->name.length, (const char *) myIntegerName->name.data);
}
...
static UA_StatusCode readTimeData(void *handle, UA_Boolean sourceTimeStamp,
  ...
  printf("readTimeData *currentTime=%ld\n", *currentTime);
	return UA_STATUSCODE_GOOD;
}
...
/*      Only on Linux        */
/*****************************/
FILE* temperatureFile = "temp.murx"; // NULL;
static UA_StatusCode readTemperature(void *handle, UA_Boolean sourceTimeStamp,
  ...
  printf("readTemperature *currentTemperature=%f\n", *currentTemperature);
...
#define ARRAYID 50002
   ...
   for(UA_UInt32 type = 0; UA_IS_BUILTIN(type); type++) {
       ...
       //add a scalar node for every built-in type
        ...
        //printIntegerName(&myIntegerName);
        UA_Server_addVariableNode(server, variant, myIntegerName, 
                                  //myIntegerNodeId, parentNodeId, parentReferenceNodeId);
                                  UA_NODEID_NUMERIC(1, ++id), UA_NODEID_NUMERIC(1, SCALARID), UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES));
        //printf("id=%d\n", id);
