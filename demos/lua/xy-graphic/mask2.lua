...
function showMask2(p)
  ...
  widgetType[2] = pv.TQwtPlotWidget

  pv.pvStartDefinition(p,ID_END_OF_WIDGETS)

  pv.pvQPushButton(p,pushButtonDraw,0)
  pv.pvSetGeometry(p,pushButtonDraw,10,10,111,40)
  pv.pvSetText(p,pushButtonDraw,"Goto gDraw")
  pv.pvSetFont(p,pushButtonDraw,"Sans Serif",10,0,0,0,0)

  pv.pvQwtPlotWidget(p,qwtWidget,0,10,1000)
  pv.pvSetGeometry(p,qwtWidget,15,60,725,465)
  pv.qpwSetCanvasBackground(p,qwtWidget,224,223,222)
  pv.qpwEnableAxis(p,qwtWidget,pv.yLeft)
  pv.qpwEnableAxis(p,qwtWidget,pv.xBottom)
  pv.pvSetFont(p,qwtWidget,"Sans Serif",10,0,0,0,0)


  pv.pvEndDefinition(p);
  --- end construction of our mask ---------------------------------------------------
  --- end variables that are private to this mask ------------------------------------
  dofile("mask2_slots.lua")                   -- include our slot functions
