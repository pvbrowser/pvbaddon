...
ParseLine(SoapCtx * ctx, char *Buffer, int LineLen)
{
  ...
  /* add to the request */
  xmlret = soap_env_add_item(ctx->env, Line, FirstCommaPos + 1, SecondCommaPos + 1);
...
{
  SoapCtx *ctx, *ctx2;
  xmlNodePtr xmlcur,xmlcur2,xmlitem,xmlitem2;
  herror_t err;
  ...
  if(strcmp(argv[3],"TestRead") == 0)
  {
     /* create a SoapCtx object */
     err = soap_ctx_new_with_method(argv[2], "Read", &ctx);
     if (err != H_OK)
     {
       log_error4("%s():%s [%d]", herror_func(err),
                  herror_message(err), herror_code(err));
     ...
     /* create the ItemList */
     strcpy(Buffer,"xsd:element,ItemList,");
     xmlitem = ParseLine(ctx, Buffer, strlen(Buffer));

     xmlcur = ctx->env->cur;
     ctx->env->cur = xmlitem;
     soap_env_add_item(ctx->env, "xsd:string", "Items ItemName=\"Pump2/Capacity\"", NULL);
     soap_env_add_item(ctx->env, "xsd:string", "Items ItemName=\"Pump2/ThroughPut\"", NULL);
     ctx->env->cur = xmlcur;
  }
  else if(strcmp(argv[3],"TestWrite") == 0)
  {
     /* create a SoapCtx object */
     err = soap_ctx_new_with_method(argv[2], "Write", &ctx);
     if (err != H_OK)
     {
       log_error4("%s():%s [%d]", herror_func(err),
                  herror_message(err), herror_code(err));
     ...
     if (!xmlNewProp(xmlitem2, BAD_CAST "ItemName", BAD_CAST "Pump2/Capacity"))
     {
        log_error1("Can not create new xml attribute ItemName");
        return 1;
     }
     ctx->env->cur = xmlitem2;
     soap_env_add_item(ctx->env,"xsd:double","Value","1234");
     ctx->env->cur = xmlitem;
     xmlitem2 = soap_env_add_item(ctx->env, "xsd:string", "Items", NULL);
     if (!xmlNewProp(xmlitem2, BAD_CAST "ItemName", BAD_CAST "Pump2/ThroughPut"))
     {
        log_error1("Can not create new xml attribute ItemName");
        return 1;
     }
     ctx->env->cur = xmlitem2;
     soap_env_add_item(ctx->env,"xsd:double","Value","5678");
     ctx->env->cur = xmlcur;
  }
  else
  {
